    ...
    if should_fit:
        ...
        for block_key_row in block_keys.iter_rows(named=True):
            ...
            try:
                ...
                _x_0_ci_lower = float(
                    __import__("numpy").quantile(threshold_samples, 0.05)
                )
                _x_0_ci_upper = float(
                    __import__("numpy").quantile(threshold_samples, 0.95)
                )
    ...
    if isinstance(selected_blocks_df, pl.DataFrame) and len(selected_blocks_df) > 0:
        ...
        for block_row in blocks_df.iter_rows(named=True):
            ...
            if gt_params is not None:
                gt_intercept = (
                    -gt_params["x_0"] / gt_params["k"] if gt_params["k"] != 0 else 0.0
                )
